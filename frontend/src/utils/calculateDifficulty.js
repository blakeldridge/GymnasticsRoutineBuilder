...
function calculateFloor(routine){
    ...
    for (let skill of routine) {
        ...
        if (skill.connection) {
            if (((skill.difficulty == 0.2 || skill.difficulty == 0.3) && routine[routine.indexOf(skill) + 1].difficulty >= 0.4) ||
                (skill.difficulty >= 0.4 && (routine[routine.indexOf(skill) + 1].difficulty == 0.2 || routine[routine.indexOf(skill) + 1].difficulty == 0.3))){
                bonus += 0.1;
    ...
    // groups 2,3,4 is 0.3 requirement for A, B, C and 0.5 for D+
    for (let i = 0; i < 3; i++) {
    ...
    //requirement_breakdown[3] = routine[routine.length - 1].difficulty;
    //requirement += routine[routine.length - 1].difficulty;
...
function calculateHbar(routine) {
    ...
    for (let skill of routine) {
        ...
        if (skill.connection) {
            if ((skill.difficulty == 0.3 && routine[routine.indexOf(skill) + 1].difficulty >= 0.4) || 
            (skill.difficulty == 0.4 && routine[routine.indexOf(skill) + 1].difficulty == 0.4) || 
            (skill.difficulty == 0.4 && routine[routine.indexOf(skill) + 1].difficulty == 0.3)){
                bonus += 0.1;
                bonus_breakdown.push([routine.indexOf(skill), routine.indexOf(skill) + 1, 0.1]);
            } else if ((skill.difficulty >= 0.4 && routine[routine.indexOf(skill) + 1].difficulty > 0.4) | 
            (skill.difficulty > 0.4 && routine[routine.indexOf(skill) + 1].difficulty >= 0.4)) {
